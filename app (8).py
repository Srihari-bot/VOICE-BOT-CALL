...
LANGUAGES = [
    ...
    {"code": "en-IN", "name": "English (India)", "flag": "🇮🇳", "voice_male": "pNInz6obpgDQGcFmaJgB", "voice_female": "EXAVITQu4vr4xnSDxMaL", "provider": "elevenlabs"},  # Adam (male), Bella (female)
    {"code": "hi-IN", "name": "Hindi (हिंदी)", "flag": "🇮🇳", "voice_male": "pNInz6obpgDQGcFmaJgB", "voice_female": None, "provider": "elevenlabs"},  # Adam (multi-lingual)
    {"code": "ta-IN", "name": "Tamil (தமிழ்)", "flag": "🇮🇳", "voice_male": "pNInz6obpgDQGcFmaJgB", "voice_female": "V9LCAAi4tTlqe9JadbCo", "provider": "elevenlabs"},  # Adam (male), custom female voice
    {"code": "te-IN", "name": "Telugu (తెలుగు)", "flag": "🇮🇳", "voice_male": "pNInz6obpgDQGcFmaJgB", "voice_female": None, "provider": "elevenlabs"},  # Adam (multi-lingual)
    {"code": "bn-IN", "name": "Bengali (বাংলা)", "flag": "🇮🇳", "voice_male": "pNInz6obpgDQGcFmaJgB", "voice_female": None, "provider": "elevenlabs"},  # Adam (multi-lingual)
    {"code": "mr-IN", "name": "Marathi (मराठी)", "flag": "🇮🇳", "voice_male": "pNInz6obpgDQGcFmaJgB", "voice_female": None, "provider": "elevenlabs"},  # Adam (multi-lingual)
    {"code": "gu-IN", "name": "Gujarati (ગુજરાતી)", "flag": "🇮🇳", "voice_male": "pNInz6obpgDQGcFmaJgB", "voice_female": None, "provider": "elevenlabs"},  # Adam (multi-lingual)
    {"code": "kn-IN", "name": "Kannada (ಕನ್ನಡ)", "flag": "🇮🇳", "voice_male": "pNInz6obpgDQGcFmaJgB", "voice_female": None, "provider": "elevenlabs"},  # Adam (multi-lingual)
    {"code": "ml-IN", "name": "Malayalam (മലയാളം)", "flag": "🇮🇳", "voice_male": "pNInz6obpgDQGcFmaJgB", "voice_female": None, "provider": "elevenlabs"},  # Adam (multi-lingual)
    
    # International Languages (Twilio Polly)
    {"code": "en-US", "name": "English (US)", "flag": "🇺🇸", "voice_male": "Polly.Matthew", "voice_female": "Polly.Joanna", "provider": "twilio"},
    {"code": "en-GB", "name": "English (UK)", "flag": "🇬🇧", "voice_male": "Polly.Brian", "voice_female": "Polly.Amy", "provider": "twilio"},
    {"code": "es-ES", "name": "Spanish (Español)", "flag": "🇪🇸", "voice_male": "Polly.Enrique", "voice_female": "Polly.Conchita", "provider": "twilio"},
    {"code": "fr-FR", "name": "French (Français)", "flag": "🇫🇷", "voice_male": "Polly.Mathieu", "voice_female": "Polly.Celine", "provider": "twilio"},
    {"code": "de-DE", "name": "German (Deutsch)", "flag": "🇩🇪", "voice_male": "Polly.Hans", "voice_female": "Polly.Marlene", "provider": "twilio"},
    {"code": "pt-BR", "name": "Portuguese (Brazil)", "flag": "🇧🇷", "voice_male": "Polly.Ricardo", "voice_female": "Polly.Vitoria", "provider": "twilio"}
]
...
def main():
    ...
    with st.container():
        ...
        new_selected_language = LANGUAGES[selected_lang_index]["code"]
        
        # Clear cached audio when language changes
        if 'prev_selected_language' not in st.session_state:
            st.session_state.prev_selected_language = new_selected_language
        elif st.session_state.prev_selected_language != new_selected_language:
            st.session_state.cached_audio_url = None
            st.session_state.cached_audio_file = None
            st.session_state.prev_selected_language = new_selected_language
        
        st.session_state.selected_language = new_selected_language
        selected_lang_obj = LANGUAGES[selected_lang_index]
        
        # Voice gender selection (only show if female voice is available)
        if selected_lang_obj.get("voice_female"):
            ...
            st.session_state.voice_gender = selected_gender_display.lower()
            # Clear cached audio when voice gender changes
            if 'prev_voice_gender' not in st.session_state:
                st.session_state.prev_voice_gender = st.session_state.voice_gender
            elif st.session_state.prev_voice_gender != st.session_state.voice_gender:
                st.session_state.cached_audio_url = None
                st.session_state.cached_audio_file = None
                st.session_state.prev_voice_gender = st.session_state.voice_gender
        else:
            st.session_state.voice_gender = "male"
            st.session_state.prev_voice_gender = "male"
        
        # Select voice based on gender
        if st.session_state.voice_gender == "female" and selected_lang_obj.get("voice_female"):
            selected_voice = selected_lang_obj["voice_female"]
        else:
            selected_voice = selected_lang_obj.get("voice_male") or selected_lang_obj.get("voice_female")
        st.session_state.selected_voice_id = selected_voice
    ...
    if send_button:
            if len(st.session_state.phone_numbers) == 0:
                ...
                for i, number in enumerate(st.session_state.phone_numbers):
                    ...
                    # Select voice based on gender / selection
                    voice_to_use = st.session_state.get("selected_voice_id")
                    if not voice_to_use:
                        if st.session_state.voice_gender == "female" and selected_lang_obj and selected_lang_obj.get("voice_female"):
                            voice_to_use = selected_lang_obj["voice_female"]
                        else:
                            voice_to_use = (selected_lang_obj.get("voice_male") if selected_lang_obj else None) or "alice"
